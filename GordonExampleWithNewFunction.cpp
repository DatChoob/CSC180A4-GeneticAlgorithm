...
void createNewRandomChild(int childIndex)
{
    ...
    new_pool[childIndex].fitness = evaluate(new_pool[childIndex].value);
}
...
int decodeInNewPool(int index)
{
  int value = 0;
  for (int i=0; i<CHROM_LENGTH; i++)
    value += (int) pow(2.0,(double)i) * new_pool[index].string[CHROM_LENGTH-1-i];
  return (value);
}
/*********************************************************
   F(X) = .1absX - sinX
